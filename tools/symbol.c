...
static int tcp_init_sock_len = 3;

static struct symbol_pattern patterns[] = {
    {"panic", panic_pattern, panic_len, panic_mask},
    {"memblock_reserve", memblock_reserve_pattern, memblock_reserve_len, memblock_reserve_mask},
    {"memblock_free", memblock_free_pattern, memblock_free_len, memblock_free_mask},
    {"memblock_alloc_try_nid", memblock_alloc_try_nid_pattern, memblock_alloc_try_nid_len, memblock_alloc_try_nid_mask},
    {"rest_init", rest_init_pattern, rest_init_len, rest_init_mask},
    {"kernel_init", kernel_init_pattern, kernel_init_len, kernel_init_mask},
    {"copy_process", copy_process_pattern, copy_process_len, copy_process_mask},
    {"avc_denied", avc_denied_pattern, avc_denied_len, avc_denied_mask},
    {"slow_avc_audit", slow_avc_audit_pattern, slow_avc_audit_len, slow_avc_audit_mask},
    {"input_handle_event", input_handle_event_pattern, input_handle_event_len, input_handle_event_mask},
    {"cgroup_init", cgroup_init_pattern, cgroup_init_len, cgroup_init_mask},
    {"cgroup_post_fork", cgroup_post_fork_pattern, cgroup_post_fork_len, cgroup_post_fork_mask},
    {"tcp_init_sock", tcp_init_sock_pattern, tcp_init_sock_len, tcp_init_sock_mask},
    {NULL, NULL, 0, NULL}
};
...
static uint64_t scan_pattern_in_range(char *base, uint32_t *pattern, uint32_t *mask, int pattern_len) {
    ...
    uint32_t *end = (uint32_t *)(base + 0x2800000);
...
int fillin_map_symbol(kallsym_t *kallsym, char *img_buf, map_symbol_t *symbol, int32_t target_is_be)
{
    ...
    tools_logi("memblock_reserve: 0x%x\n", symbol->memblock_reserve_relo);
    tools_logi("memblock_free: 0x%x\n", symbol->memblock_free_relo);
    tools_logi("memblock_alloc: 0x%x\n", symbol->memblock_phys_alloc_relo);
